...
const SubscriptionPage = () => {
  ...
  const fetchPaymentMethods = async () => {
    try {
      ...
      // Metode pembayaran manual (dari localStorage jika ada)
      let manualMethods = [];
      if (manualMethodsStr) {
        try {
      ...
      // Metode pembayaran Tripay (akan diganti dengan API call ke Tripay)
      const tripayMethods = [
      ...
      // Gabungkan metode Tripay dan manual
      let allMethods = [];
      
      // Jika Tripay diaktifkan, tambahkan metode Tripay
      if (tripayEnabledSetting !== 'false') {
        allMethods = [...tripayMethods];
      }
      
      // Tambahkan metode manual dengan penanda isManual
  ...
  const renderPaymentInstructions = (transaction) => {
    ...
    const { payment_method, qr_url, payment_code, instructions, isManual, manualData } = transaction;
    ...
    if (isManual && manualData) {
      return (
        <div style={{ marginTop: 20 }}>
          ...
          {manualData.type === 'qris' && manualData.qrImageUrl && (
            <div style={{ textAlign: 'center' }}>
              <img 
                src={manualData.qrImageUrl} 
                alt="QRIS Code" 
          ...
          {(manualData.type === 'bank' || manualData.type === 'ewallet') && (
            <div>
              <Alert
                message={manualData.type === 'bank' ? "Rekening Tujuan" : "Akun Tujuan"}
                description={
                  <>
                    <Text copyable strong style={{ fontSize: '16px' }}>
                      {manualData.accountNumber}
                    </Text>
                    <div style={{ marginTop: 8 }}>
                      a/n {manualData.accountName}
                    </div>
                  </>
                }
                type="info"
          ...
          {manualData.instructions && (
            <Alert
              message="Petunjuk Pembayaran"
              description={manualData.instructions}
              type="info"
              showIcon
              style={{ marginBottom: 20 }}
            />
