  ...
  try {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        const activeMethods = response.data.filter(method => method.active);
        setPaymentMethods(activeMethods);
        return activeMethods;
      }
    } catch (apiErr) {
      console.error('Error fetching from API, falling back to tripay status check:', apiErr);
    }
    
    // Fallback: cek status Tripay dan gunakan metode default
    try {
      const tripayStatusResponse = await axiosInstance.get('/api/settings/tripay-status');
      const tripayEnabled = tripayStatusResponse.data.enabled;
      
      console.log('Tripay enabled?', tripayEnabled);
      
      // Jika Tripay aktif, gunakan metode pembayaran default
      if (tripayEnabled) {
        const defaultMethods = [
          { code: 'QRIS', name: 'QRIS', type: 'qris', fee: 800, active: true },
          { code: 'BRIVA', name: 'Bank BRI', type: 'bank', fee: 4000, active: true },
          { code: 'MANDIRIVA', name: 'Bank Mandiri', type: 'bank', fee: 4000, active: true },
          { code: 'BNIVA', name: 'Bank BNI', type: 'bank', fee: 4000, active: true },
          { code: 'BCAVA', name: 'Bank BCA', type: 'bank', fee: 4000, active: true },
          { code: 'OVO', name: 'OVO', type: 'ewallet', fee: 2000, active: true },
          { code: 'DANA', name: 'DANA', type: 'ewallet', fee: 2000, active: true },
          { code: 'LINKAJA', name: 'LinkAja', type: 'ewallet', fee: 2000, active: true },
          { code: 'SHOPEEPAY', name: 'ShopeePay', type: 'ewallet', fee: 2000, active: true }
        ];
        
        setPaymentMethods(defaultMethods);
        return defaultMethods;
      } else {
        console.log('Tripay is disabled, no payment methods available');
        setPaymentMethods([]);
        return [];
      }
    } catch (statusErr) {
      console.error('Error fetching Tripay status, assuming disabled:', statusErr);
      setPaymentMethods([]);
      return [];
    }
  } catch (err) {
...
>
  {useDetailedPaymentView ? (
    <Radio.Group>
      {/* Bank Transfer */}
      {paymentMethods.filter(method => method.type === 'bank' && method.active).length > 0 && (
        <>
          <div style={{ marginBottom: 8 }}><Text strong>Transfer Bank</Text></div>
          <Row gutter={[8, 8]}>
            {paymentMethods
              .filter(method => method.type === 'bank' && method.active)
              .map(method => (
      ...
      {/* E-Wallet */}
      {paymentMethods.filter(method => method.type === 'ewallet' && method.active).length > 0 && (
        <>
          <div style={{ margin: '16px 0 8px' }}><Text strong>E-Wallet</Text></div>
          <Row gutter={[8, 8]}>
            {paymentMethods
              .filter(method => method.type === 'ewallet' && method.active)
              .map(method => (
                <Col span={12} key={method.code}>
                  <Radio.Button 
                    value={method.code}
      ...
      {/* QRIS */}
      {paymentMethods.filter(method => method.type === 'qris' && method.active).length > 0 && (
        <>
          <div style={{ margin: '16px 0 8px' }}><Text strong>QRIS</Text></div>
          <Row gutter={[8, 8]}>
            {paymentMethods
              .filter(method => method.type === 'qris' && method.active)
              .map(method => (
                <Col span={12} key={method.code}>
                  <Radio.Button 
                    value={method.code}
    ...
    /* Tampilan Sederhana (inline buttons) */
